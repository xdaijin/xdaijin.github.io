...
上面的方案要么不合理，要么实现复杂，先删除缓存后更新数据库基本没有必要，因为数据延迟就算不存在数据库中也可能存在客户端中。
而先更新数据库后删除缓存的方式可能存在更新数据库后程序报错导致没有更新缓存的问题，解决的办法就是，在更新数据之前，先给数据设置
一个较短的超时时间，这样就算更新数据库之前报错和更新数据库之后报错都没有问题，顶多存在延迟
